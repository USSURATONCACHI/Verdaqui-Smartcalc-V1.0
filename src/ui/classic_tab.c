...
void classic_tab_free(ClassicTab tab) { nk_textedit_free(&tab.expr_text); }
...
void classic_tab_draw(ClassicTab* this, struct nk_context* ctx,
  ...
  nk_spacer(ctx);
  nk_layout_row_dynamic(ctx, 50, 6);
  const char* const buttons_2[] = {"sin", "cos", "tan", "asin", "acos", "atan"};
...
static void default_button(ClassicTab* this, struct nk_context* ctx,
  ...
  if (nk_button_label(ctx, text)) {
    ...
    if (strcmp(text, "C") is 0) {
      ...
      char last_char =
          len > 0 ? nk_str_get_const(&this->expr_text.string)[len - 1] : '\0';
      bool is_space = last_char is ' ';
      bool is_num = (last_char >= '0' and last_char <= '9') or last_char is '.';
      bool current_is_num =
          (text[0] >= '0' and text[0] <= '9') or text[0] is '.';
...
void classic_tab_update(ClassicTab* this) { unused(this); }
