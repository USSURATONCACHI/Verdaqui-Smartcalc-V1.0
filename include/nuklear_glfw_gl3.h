...
NK_API void                 nk_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods);
...
NK_API void
nk_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) 
{
    struct nk_glfw* glfw = glfwGetWindowUserPointer(window);
    if (action == GLFW_PRESS || action == GLFW_REPEAT)
        if (glfw->keys_len < NK_GLFW_TEXT_MAX)
            glfw->keys[glfw->keys_len++] = key;
...
nk_glfw3_new_frame(struct nk_glfw* glfw)
{
    ...
    const int KEYS_MAPPING[] = {
        ...
        NK_KEY_SHIFT,           GLFW_KEY_LEFT_SHIFT,
        NK_KEY_SHIFT,           GLFW_KEY_RIGHT_SHIFT,
    };
    const int KEYS_MAPPING_LEN = sizeof(KEYS_MAPPING) / sizeof(KEYS_MAPPING[0]) / 2;
    
    for (i = 0; i < glfw->keys_len; ++i)
        for (int map = 0; map < KEYS_MAPPING_LEN; map++)
            if (glfw->keys[i] == KEYS_MAPPING[map * 2 + 1])
                nk_input_key(ctx, KEYS_MAPPING[map * 2], glfwGetKey(win, KEYS_MAPPING[map * 2 + 1]) == GLFW_PRESS);
    ...
    if (glfwGetKey(win, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS ||
        ...
        const int CTRL_KEYS_MAPPING_LEN = sizeof(CTRL_KEYS_MAPPING) / sizeof(CTRL_KEYS_MAPPING[0]) / 2;
        
        for (i = 0; i < glfw->keys_len; ++i)
            for (int map = 0; map < CTRL_KEYS_MAPPING_LEN; map++)
                if (glfw->keys[i] == CTRL_KEYS_MAPPING[map * 2 + 1])
                    nk_input_key(ctx, CTRL_KEYS_MAPPING[map * 2], glfwGetKey(win, CTRL_KEYS_MAPPING[map * 2 + 1]) == GLFW_PRESS);
        /*
        ...
        const int NO_CTRL_KEYS_MAPPING[] = {
            NK_KEY_LEFT, GLFW_KEY_LEFT,
            NK_KEY_RIGHT, GLFW_KEY_RIGHT,
        };
        const int NO_CTRL_KEYS_MAPPING_LEN = sizeof(NO_CTRL_KEYS_MAPPING) / sizeof(NO_CTRL_KEYS_MAPPING[0]) / 2;
        
        for (i = 0; i < glfw->keys_len; ++i)
            for (int map = 0; map < NO_CTRL_KEYS_MAPPING_LEN; map++)
                if (glfw->keys[i] == NO_CTRL_KEYS_MAPPING[map * 2 + 1])
                    nk_input_key(ctx, NO_CTRL_KEYS_MAPPING[map * 2], glfwGetKey(win, NO_CTRL_KEYS_MAPPING[map * 2 + 1]) == GLFW_PRESS);
